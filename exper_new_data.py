...
from my_stuff2 import *

file_name = input('input ur log name: ')
num_epoch = 20
for IND in range(2):
    BS = 32#256-64
    ...
    for size_char in max_size_char:
        ...
        data_tr = MyDataloader('clean_news_en_tr'+ str(IND) +'.txt', 'label_news_en_tr'+ str(IND) +'.txt',\
                               RULEs, word_length, '|', 'char_vec_dictionary.txt', size_char, \
                               'fasttext.th.vec', 300, device, 'pos_news_en_tr'+ str(IND) +'.txt',POSMAP)
        data_te = MyDataloader('clean_news_en_te'+ str(IND) +'.txt', 'label_news_en_te'+ str(IND) +'.txt', \
                               RULEs, word_length, '|', 'char_vec_dictionary.txt', size_char, \
                               'fasttext.th.vec', 300, device, 'pos_news_en_te'+ str(IND) +'.txt',POSMAP)
        ...
        for i in grucrf_dropout:
            ...
            best_cnt_mat = np.zeros((len(tags)-1,3))
            best_epoch = 0
            cnt_idle = 0
            ...
            with open(file_name + '.txt', 'a', encoding='utf8') as f:
                ...
                f.write(f'cnt_mat\n')
                f.write(f'I => : {best_cnt_mat[0,0]}, : {best_cnt_mat[0,1]}, : {best_cnt_mat[0,2]}\n')
                f.write(f'B => : {best_cnt_mat[1,0]}, : {best_cnt_mat[1,1]}, : {best_cnt_mat[1,2]}\n')
                f.write(f'O => : {best_cnt_mat[2,0]}, : {best_cnt_mat[2,1]}, : {best_cnt_mat[2,2]}\n')
