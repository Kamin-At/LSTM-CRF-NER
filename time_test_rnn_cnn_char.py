...
for BS in list_BS:
    ...
    for size_char in max_size_char:
        ...
        tr, te = get_indices_random_val_test_split(len(data_tr), 1, 0.1, True)
...
#                  i, True, tags, i, pos_size, FCN, 0.5)
            ...
            for epoch in range(num_epoch):
                ...
                total_loss = sum(all_loss)/(ind + 1)
                my_scheduler.step(total_loss)
            with open(f'log_time_cnn_rnn_char_BS_{BS}_char_size_{size_char}.txt', 'w', encoding = 'utf8') as f:
                f.write('CNN, RNN\n')
